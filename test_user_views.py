...
class MessageViewTestCase(TestCase):
    ...
    def setup_likes(self):
        ...
        m3 = Message(id=9877, text="likable warble", user_id=self.u1_id)
        db.session.add_all([m1, m2, m3])
        db.session.commit()

        l1 = Likes(user_id=self.testuser_id, message_id=9877)
    ...
    def test_remove_like(self):
        ...
        with self.client as c:
            ...
            resp = c.post(
                f"/messages/{m.id}/remove_like", follow_redirects=True)
            self.assertEqual(resp.status_code, 200)
